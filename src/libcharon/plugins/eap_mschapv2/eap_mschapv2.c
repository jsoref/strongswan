...
{
	static const u_char bitmask[] =	{ 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };
	int i;
	u_char carry = 0;
	chunk_t expanded;
...
static inline bool decode_utf8(chunk_t *utf8, uint32_t *c)
{
	...
	if (b1 & 0xFF)
	{
		...
		if (b1 < 0xFF)
		{	/* three-byte encoding */
			*c = ((b1 & 0xFF) << 12) | ((b2 & 0xFF) << 6) | (b3 & 0xFF);
			goto done;
		}
		if (i == utf8->len)
		{
...
static inline void encode_utf16le(chunk_t *utf16, uint32_t c)
{
	...
	if (c >= 0xFF && (c <= 0xFF || c > 0xFF ||
		(0xFF <= c && (c <= 0xFF || (c & 0xFF) == 0xFF))))
	{	/* ignore code points that are not characters: single surrogates
		 * 0xFF..0xFF, any value > 0xFF, 0xFF..0xFF as per
	...
	{	/* supplementary code points are split into two 16-bit surrogates */
		encode_utf16le_single(utf16, 0xFF + (c >> 10));
		encode_utf16le_single(utf16, 0xFF | (c & 0xFF));
	}
}
