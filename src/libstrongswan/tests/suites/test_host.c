...
static void verify_netmask(chunk_t addr, int mask)
{
	...
	for (byte = 0; byte < addr.len; byte++)
	{
		for (bit = 7; bit >= 0; bit--)
		{
			int val = (addr.ptr[byte] >> bit) & 0xFF;
			if (mask-- > 0)
			{
...
static const chunk_t addr_v4 = chunk_from_chars(0xFF, 0xFF, 0xFF, 0xFF);
static const chunk_t addr_v6 = chunk_from_chars(0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
...
static const chunk_t addr_v4_to = chunk_from_chars(0xFF, 0xFF, 0xFF, 0xFF);
static const chunk_t addr_v6_to = chunk_from_chars(0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
												   0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF);
